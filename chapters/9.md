...
Assume that you want to draw an sphere like this one:
...
from the eye?
- And what if you want to apply a blur effect to the bricks?
- And what if you want to enable or disable lighting in the sphere ?

The answer is to use a `Material` instead of just plain an simple texture. In fact,
`Material` can have one ore more textures and much much more.

In particular, a `Material` is defined by:

- one or more `Technique` objects
- each `Technique` can have one more `Pass` objects
- each `Pass` object has:
  - a `RenderState` object
  - a `Shader` object
  - zero or more `Sampler` objects
...
material spaceship
{
	...
	technique normal
	{
		...
		pass 0
		{
            ...
            // sampler: responsible for setting the texture and its parameters
			sampler 0
			{
...
Instead of using JSON or XML, Material uses a file format  optimized to create
Material files. In fact, its file format is very similar to other existing
Material file formats, like GamePlay3D's and OGRE3D's.

A Material file must start with a comment or with a `material` section.
...
and each material section must have one ore more `technique` sections:
...
and each `technique` section must have one ore more `pass` sections:

material material_name
{
    technique default
    {
        // passes are going to be executed in order
        pass first
        {
        }
        // this pass will be executed after "first"
        pass second
        {
        }
    }
...
It is recommended that all Material files end with the `.material` extension.

Each Material file could have one or more material.
